...
(function () {
    ...
    Sprite_MiniMap.prototype.redrawMap = function () {
        ...
        var padX = screenTileW * 0.20;
        var padY = screenTileH * 0.20;

        var startX = Math.floor(viewX - padX);
        var startY = Math.floor(viewY - padY);
        var endX = Math.ceil(viewX + screenTileW + padX);
        var endY = Math.ceil(viewY + screenTileH + padY);
        ...
        for (var my = startY; my < endY; my++) {
            for (var mx = startX; mx < endX; mx++) {
                ...
                var totalTileW = screenTileW + padX * 2;
                var totalTileH = screenTileH + padY * 2;
                var originX = viewX - padX;
                var originY = viewY - padY;

                var dx1 = rect.x + Math.floor(((mx - originX) / totalTileW) * rect.w);
                var dy1 = rect.y + Math.floor(((my - originY) / totalTileH) * rect.h);
                var dx2 = rect.x + Math.floor((((mx + 1) - originX) / totalTileW) * rect.w);
                var dy2 = rect.y + Math.floor((((my + 1) - originY) / totalTileH) * rect.h);
    ...
    Sprite_MiniMap.prototype.getCharacterMiniMapPos = function (character, rect) {
        ...
        var rx = (cx - (viewX - padX)) / totalTileW;
        var ry = (cy - (viewY - padY)) / totalTileH;
